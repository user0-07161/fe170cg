...
static const struct snd_soc_dapm_route ME175CG_PF400CG_ctp_audio_map[] = {
#ifdef CONFIG_EEPROM_PADSTATION
...
int ctp_set_asp_clk_fmt(struct snd_soc_dai *codec_dai)
{
	...
	ret = snd_soc_dai_set_pll(codec_dai, RT5648_SCLK_S_PLL1,
				RT5648_PLL1_S_MCLK, DEFAULT_MCLK, 24576000);
	if (ret < 0) {
...
#else
	ret = snd_soc_dai_set_sysclk(codec_dai, RT5648_SCLK_S_MCLK,
...
int ctp_init(struct snd_soc_pcm_runtime *runtime)
{
	...
	/*
        if(PROJ_ID == PROJ_ID_ME175CG || PROJ_ID == PROJ_ID_PF400CG)
        snd_soc_dapm_add_routes(dapm, ME175CG_PF400CG_ctp_audio_map,
                                        ARRAY_SIZE(ME175CG_PF400CG_ctp_audio_map));
        else
  */
			...
			struct snd_soc_jack *jack, int enable)
{
	...
	status = rt5648_headset_detect(codec, !enable); /* bard: What does enable mean? */
