...
// i2c stress test ++
#define RT5648_IOC_MAGIC 0xF3
#define RT5648_IOC_MAXNR 2
#define RT5648_POLL_DATA _IOR(RT5648_IOC_MAGIC,2,int )

#define RT5648_IOCTL_START_HEAVY 2
#define RT5648_IOCTL_START_NORMAL 1
#define RT5648_IOCTL_END 0

#define START_NORMAL	(HZ)
#define START_HEAVY 	(HZ)

static int poll_mode=0;
struct delayed_work rt5648_poll_data_work;
static struct workqueue_struct *sensor_work_queue;
struct miscdevice rt5648_misc_dev;
struct snd_soc_codec *i2c_test_codec;
// --
...
static struct rt5648_init_reg init_list[] = {
	...
	{ RT5648_SPO_MIXER	, 0xc807 },/* SPKVOL -> SPO */ /* unmute SPKVOLR -> SPOLMIX for single speaker */
#endif
...
static const struct snd_kcontrol_new rt5648_snd_controls[] = {
	...
	SOC_SINGLE_TLV("IN1 Boost", RT5648_CJ_CTRL1,
		RT5648_CBJ_BST1_SFT, 8, 0, bst_tlv),
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		rt5648_update_eqmode(codec, EQ_CH_DAC, rt5648->eq_mode);
		...
		//jessica_chou: add for auto ratio
	       rt5648_index_update_bits(codec, RT5648_CLSD_INT_REG1, 0x0010, 0x0010);

	break;

	case SND_SOC_DAPM_PRE_PMD:
		rt5648_update_eqmode(codec, EQ_CH_DAC, NORMAL);
		/*snd_soc_update_bits(codec, RT5648_SPK_VOL,
		...
		snd_soc_update_bits(codec, RT5648_GEN_CTRL3, (0x3 << 9), 0);
             ...
             //jessica_chou: add for auto ratio
		rt5648_index_update_bits(codec, RT5648_CLSD_INT_REG1, 0x0010, 0); 
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		snd_soc_write(codec, RT5648_ADJ_HPF2, 0x0909);
		break;

	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_write(codec, RT5648_ADJ_HPF2, 0x0);
		break;
	default:
		return 0;
	}
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_write(codec, RT5648_ALC_CTRL_4, 0x6014);
		snd_soc_write(codec, RT5648_ALC_CTRL_1, 0xc22c);
		break;
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_update_bits(codec, RT5648_PWR_ANLG2,
	...
	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_update_bits(codec, RT5648_ALC_CTRL_1,
			RT5648_DRC_AGC_MASK, RT5648_DRC_AGC_DIS);
		snd_soc_update_bits(codec, RT5648_PWR_ANLG2,
			RT5648_PWR_BST2_P, 0);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	struct rt5648_priv *rt5648 = snd_soc_codec_get_drvdata(codec);
      //jessica_chou: remove EQ enabled for SPK only EQ
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
...
static int rt5648_sto_filter_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	struct rt5648_priv *rt5648 = snd_soc_codec_get_drvdata(codec);
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
	//	if (snd_soc_read(codec, RT5648_PWR_DIG1) & RT5648_PWR_CLS_D) {
			//rt5648_update_eqmode(codec, EQ_CH_DAC, rt5648->eq_mode);
	//	} 
...
static const struct snd_soc_dapm_widget rt5648_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SUPPLY("dac stereo1 filter", RT5648_PWR_DIG2,
		RT5648_PWR_DAC_S1F_BIT, 0, rt5648_sto_filter_event, 
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
  ...
  //jessica_chou: disable auto ratio if in Call
  if (is_csv_call_active())
    snd_soc_update_bits(codec, RT5648_CLSD_OUT_CTRL, 0x0300, 0x0200);
  else
    snd_soc_update_bits(codec, RT5648_CLSD_OUT_CTRL, 0x0300, 0x0300);
  pr_debug("call statue : %d", is_csv_call_active());
...
static int rt5648_probe(struct snd_soc_codec *codec)
{
	...
	pr_info("FE170CG alc5648 Codec driver version %s\n", VERSION);

	i2c_test_codec = codec;
 ...
 * i2c stress test ++
 */
int rt5648_open(struct inode *inode, struct file *filp)
{
	pr_debug("clovertrail audio info : %s\n", __func__);
	return 0;
}

int rt5648_release(struct inode *inode, struct file *filp)
{
	pr_debug("clovertrail audio info : %s\n", __func__);
	return 0;
}

int rt5648_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int err = 1;

	if (_IOC_TYPE(cmd) != RT5648_IOC_MAGIC)
	return -ENOTTY;
	if (_IOC_NR(cmd) > RT5648_IOC_MAXNR)
	return -ENOTTY;
	...
	switch (cmd) {
		case RT5648_POLL_DATA:
			if (arg == RT5648_IOCTL_START_HEAVY){
				pr_debug("clovertrail audio info : ioctl heavy\n");
				poll_mode = START_HEAVY;
				queue_delayed_work(sensor_work_queue, &rt5648_poll_data_work, poll_mode);
			}
			else if (arg == RT5648_IOCTL_START_NORMAL){
				pr_debug("clovertrail audio info : ioctl normal\n");
				poll_mode = START_NORMAL;
				queue_delayed_work(sensor_work_queue, &rt5648_poll_data_work, poll_mode);
			}
			else if  (arg == RT5648_IOCTL_END){
				pr_debug("clovertrail audio info : ioctl end\n");
				cancel_delayed_work_sync(&rt5648_poll_data_work);
			}
			else
				return -ENOTTY;
			break;
...
struct file_operations rt5648_fops = {
	.owner = THIS_MODULE,
	.unlocked_ioctl = rt5648_ioctl,
	.open = rt5648_open,
	.release = rt5648_release,
};

static void  rt5648_poll_data(struct work_struct * work)
{
	int lux = snd_soc_read(i2c_test_codec, 0);
	pr_debug("FOR TEST , rt5648_poll_data clovertrail audio lux = %d\n",lux);
	if(poll_mode ==0)
		msleep(5);
		    ...
		    const struct i2c_device_id *id)
{
	...
	sensor_work_queue = create_singlethread_workqueue("i2c_clovertrail_wq");
	if(!sensor_work_queue){
		pr_err("rt5648_probe: Unable to create workqueue");
		goto exit_i2c;
	}

	INIT_DELAYED_WORK(&rt5648_poll_data_work, rt5648_poll_data);
	rt5648_misc_dev.minor  = MISC_DYNAMIC_MINOR;
	rt5648_misc_dev.name = "clovertrailaudio";
	rt5648_misc_dev.fops = &rt5648_fops;
	ret = misc_register(&rt5648_misc_dev);
	if (ret){
		pr_err("clovertrail audio err : Unable to register %s misc device\n",
				rt5648_misc_dev.name);
		goto exit_i2c;
	}
        //--	
exit_i2c:        
...
{
      misc_deregister(&rt5648_misc_dev);
