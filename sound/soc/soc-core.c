...
//jessica_chou: add for audio system resume time retrieval
struct timeval resume_time_tv;
...
long latest_resume_time(void) {
        return resume_time;
}

EXPORT_SYMBOL(latest_resume_time);
...
int snd_soc_resume(struct device *dev)
{
	...
	do_gettimeofday(&resume_time_tv);
	resume_time = ((long)resume_time_tv.tv_sec)*1000+(long)resume_time_tv.tv_usec/1000;
