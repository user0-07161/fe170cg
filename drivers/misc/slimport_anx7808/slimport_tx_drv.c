...
static void sp_tx_link_phy_initialization(void)
{
	...
	sp_write_reg(TX_P1, SP_TX_LT_CTRL_REG0, 0x1c); //before 0x19
	sp_write_reg(TX_P1, SP_TX_LT_CTRL_REG4, 0x15);
	sp_write_reg(TX_P1, SP_TX_LT_CTRL_REG7, 0x27);
	...
	sp_write_reg(TX_P1, SP_TX_LT_CTRL_REG1, 0x2c); //before 0x26
	...
	sp_write_reg(TX_P1, SP_TX_LT_CTRL_REG2, 0x3f); //before 0x36
	sp_write_reg(TX_P1, SP_TX_LT_CTRL_REG6, 0x3f);
	sp_write_reg(TX_P1, SP_TX_LT_CTRL_REG16, 0x1f);
	sp_write_reg(TX_P1, SP_TX_LT_CTRL_REG3, 0x3F);
	sp_write_reg(TX_P1, SP_TX_LT_CTRL_REG12, 0x1f);
...
void hdmi_rx_set_hpd(unchar enable)
{
	...
	if (enable) {
		...
		hdmi_status = 1;
		SP_DEV_NOTICE("%s: ATD Tool needed hdmi_status = 1\n", __func__);
	} else {
		...
		hdmi_status = 0;
		SP_DEV_NOTICE("%s: ATD Tool needed hdmi_status = 0\n", __func__);
	}
}
