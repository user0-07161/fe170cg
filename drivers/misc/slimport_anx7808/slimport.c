...
void sp_tx_hardware_poweron(void)
{
	...
	if(pdata->power_on) pdata->power_on(1);
	msleep(20);
...
void sp_tx_hardware_powerdown(void)
{
	...
	if(pdata->power_on) pdata->power_on(0);
	msleep(5);
...
//status add for factory test {
int hdmi_status = 0;
module_param(hdmi_status, int, 0644);
//status add for factory test }
		...
		if (gpio_get_value_cansleep(anx7808->pdata->gpio_cbl_det)) {
			...
			case RX_HDMI:
				if (sp_tx_get_hdmi_connection()){
					...
					/*hdmi_status = 1;
					SP_DEV_DBG("%s: ATD Tool needed hdmi_status = 1\n", __func__);*/
				}
				/*else{
                                        hdmi_status = 0;
                                        SP_DEV_DBG("%s: ATD Tool needed hdmi_status = 0\n", __func__);
				}*/
				break;
			case RX_DP:
...
static int anx7808_init_gpio(struct anx7808_data *anx7808)
{
	...
	ret = gpio_request(anx7808->pdata->gpio_p_dwn, "MYDP_EN_N");
	if (ret) {
	...
	ret = gpio_request(anx7808->pdata->gpio_reset, "MYDP_RESETN_N");
	if (ret) {
		SP_DEV_ERR("%s : failed to request gpio %d\n", __func__,
	...
	ret = gpio_request(anx7808->pdata->gpio_int, "MYDP_INTP");
	if (ret) {
		SP_DEV_ERR("%s : failed to request gpio %d\n", __func__,
	...
	ret = gpio_request(anx7808->pdata->gpio_cbl_det, "MYDP_CBL_IN");
	if (ret) {
		SP_DEV_ERR("%s : failed to request gpio %d\n", __func__,
	...
	ret = gpio_request(anx7808->pdata->gpio_v10_ctrl, "MYDP_1V_EN");
	if (ret) {
		SP_DEV_ERR("%s : failed to request gpio %d\n", __func__,
	...
	if(anx7808->pdata->power_on) anx7808->pdata->power_on(0);
...
static irqreturn_t anx7808_cbl_det_isr(int irq, void *data)
{
	...
	if (gpio_get_value(anx7808->pdata->gpio_cbl_det)) {
		...
		wake_unlock(&anx7808->slimport_lock);
                //Thomas add for factory test {
                hdmi_status = 0;
                SP_DEV_DBG("%s: in isr : ATD Tool needed hdmi_status = 0\n", __func__);
                //Thomas add for factory test }
	}
