...
#endif
#ifdef TPUT_MONITOR
#ifndef TPUT_THRESHOLD
#define TPUT_THRESHOLD 3932160 // 30Mbit = 30*1024*1024/8
#endif
#define TPUT_CHECK_INTERVAL 2 //2s
	struct timer_list tput_timer;
	tsk_ctl_t tput_tsk;
#endif
#ifdef BTC_MONITOR
	struct timer_list btc_timer;
	tsk_ctl_t btc_tsk;
#endif
...
/* Enable 40MHz at 2.4G & 5G */
int bw_40all = 0;
module_param(bw_40all, int, 0644);
...
#ifdef TPUT_MONITOR
void dhd_set_cpu_freq(int on);
#endif

#ifdef BTC_MONITOR
static void dhd_btc_monitor_release(dhd_pub_t *dhdp);
static void dhd_btc_monitor_func(ulong data);
static int dhd_btc_monitor_thread(void *data);
#endif
	...
	while (1)
		if (down_interruptible(&tsk->sema) == 0) {
			...
			if ((tx_bytes - pre_tx_bytes) >=
				(TPUT_THRESHOLD * TPUT_CHECK_INTERVAL) ||
			...
			mod_timer(&dhd->tput_timer, jiffies + TPUT_CHECK_INTERVAL * HZ);
		} else {
			break;
		}
		...
		if (down_interruptible(&tsk->sema) == 0) {
			...
			if ((rx_packets - dhdp->pre_rx_packets) >= DYNAMIC_RX_DIFF) {
				...
				if (!dhdp->dynamic_dtim_skip &&
					...
					dhdp->dynamic_dtim_skip = dhd_get_suspend_bcn_li_dtim(dhdp);
					dhdp->dynamic_dtim_data_counter = DYNAMIC_SETUP_COUNTER;
					bcm_mkiovar("bcn_li_dtim", (char *)&dhdp->dynamic_dtim_skip,
							4, iovbuf, sizeof(iovbuf));
					dhd_wl_ioctl_cmd(dhdp, WLC_SET_VAR, iovbuf,
...
#ifdef BTC_MONITOR
	dhd_btc_monitor_release(&dhd->pub);
#endif
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	PROC_START(dhd_dtim_thread, dhd, &dhd->dtim_tsk, 0, "dhd_dtim_thread");
#endif
#ifdef TPUT_MONITOR
...
#endif
#ifdef BTC_MONITOR
	dhd->btc_tsk.thr_pid = -1;
	init_timer(&dhd->btc_timer);
	dhd->btc_timer.data = (ulong)dhd;
	dhd->btc_timer.function = dhd_btc_monitor_func;
	PROC_START(dhd_btc_monitor_thread, dhd, &dhd->btc_tsk, 0, "dhd_btc_monitor_thread");
#endif
	if (dhd_sysioc) {
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	/* set HT 40 */
	if (bw_40all && (!op_mode && strstr(fw_path, "_apsta") == NULL) && (op_mode != 0x0002)) {
		int mimo_bw_cap = WLC_N_BW_40ALL;
		bcm_mkiovar("mimo_bw_cap", (char *)&mimo_bw_cap, 4, iovbuf, sizeof(iovbuf));
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
			sizeof(iovbuf), TRUE, 0)) < 0) {
			DHD_ERROR(("%s mimo_bw_cap for HT40 failed %d\n", __FUNCTION__, ret));
		}
	}
...
#endif
#ifdef BTC_MONITOR
		if (dhd->btc_tsk.thr_pid >= 0) {
			PROC_STOP(&dhd->btc_tsk);
		}
#endif
	}
#ifdef WL_CFG80211
...
void dhd_set_cpu_freq(int on)
{
	...
	for (i = 0; i < CPU_NUM; i++) {
		...
		fp = filp_open(buf, O_WRONLY, S_IRWXU|S_IRWXG|S_IRWXO);
		if (IS_ERR(fp)) {
			DHD_ERROR(("%s: open fail, cpu = %d\n", __FUNCTION__, i));
			set_fs(old_fs);
			return -1;
		}

		if (on) {
			ret = vfs_write(fp, "performance", 1024, &fp->f_pos);
			DHD_TRACE(("%s: set cpu%d to performance\n", __FUNCTION__, i));
			if (ret < 0)
				DHD_ERROR(("%s: set performance fail %d\n", __FUNCTION__, ret));
		} else {
			ret = vfs_write(fp, "interactive", 1024, &fp->f_pos);
			DHD_TRACE(("%s: set cpu%d to interactive\n", __FUNCTION__, i));
			if (ret < 0)
				DHD_ERROR(("%s: set interactive fail %d\n", __FUNCTION__, ret));
		}
...
#ifdef BTC_MONITOR
static int btc_mode = 0;
static int
dhd_control_btc_mode(dhd_info_t *dhd, int enable)
{
	int ret = 0;
	char buf[1024];
	char btc_flag0_disable[8] ={ 0, 00, 00, 00, 0x0, 0x0, 0x00, 0x00 };
	char btc_flag0_enable[8] = { 0, 00, 00, 00, 0x1, 0x0, 0x00, 0x00};
	dhd_pub_t *dhdp = &dhd->pub;

	memset(buf, 0, sizeof(buf));

	if(enable == 0) {
		printk("%s: andy Try to disable btc\n",__func__);
		bcm_mkiovar("btc_flags", (char *)&btc_flag0_disable[0], sizeof(btc_flag0_disable), buf, sizeof(buf));
		if((ret = dhd_wl_ioctl_cmd(dhdp, WLC_SET_VAR, buf, sizeof(buf), TRUE, 0)) < 0 )
			DHD_ERROR(("%s: Disable btc fail\n", __FUNCTION__));

	} else if(enable == 1) {
		printk("%s: andy Try to enable btc\n",__func__);
		bcm_mkiovar("btc_flags", (char *)&btc_flag0_enable[0], sizeof(btc_flag0_enable), buf, sizeof(buf));
		if((ret = dhd_wl_ioctl_cmd(dhdp, WLC_SET_VAR, buf, sizeof(buf), TRUE, 0)) < 0 )
			DHD_ERROR(("%s: Enable btc fail\n", __FUNCTION__));
	}
	return ret;
}

static int
dhd_btc_monitor_thread(void *data)
{
	tsk_ctl_t *tsk = (tsk_ctl_t *)data;
	dhd_info_t *dhd = (dhd_info_t *)tsk->parent;
	int ret = 0;
	DAEMONIZE("dhd_btc_monitor");
	...
	while (1)
		if (down_interruptible(&tsk->sema) == 0) {
			...
			printk("%s: set btc mode %d\n", __func__, btc_mode);
			ret = dhd_control_btc_mode(dhd,btc_mode);
			if((btc_mode == 0) && (ret >= 0)){
				btc_mode = 1;
				mod_timer(&dhd->btc_timer, jiffies + 10 * HZ);
			}
		} else {
			break;
		}
...
static void
dhd_btc_monitor_func(ulong data)
{
	dhd_info_t *dhd = (dhd_info_t *)data;
	if (dhd->btc_tsk.thr_pid >= 0) {
		up(&dhd->btc_tsk.sema);
...
static void
dhd_btc_monitor_release(dhd_pub_t *dhdp)
{
	dhd_info_t *dhd = (dhd_info_t *)dhdp->info;
	DHD_TRACE(("%s: btc_monitor release\n", __FUNCTION__));
	del_timer_sync(&dhd->btc_timer);
}

static void btc_mode_handler(const char *kmessage, struct kernel_param *kp)
{
...
module_param_call(btc_mode, btc_mode_handler, param_get_int, &btc_mode,
