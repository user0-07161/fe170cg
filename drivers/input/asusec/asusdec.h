...
#define ASUSDEC_KEYPAD_READINGMODE		0xf6
...
/*Implement sleep mode start*/
//#define ASUSDEC_KEYPAD_SUSPEND		0x3a	//F1
#define ASUSDEC_KEYPAD_SUSPEND		0x01
/*Implement sleep mode start*/
...
#define ASUSDEC_SCI_MASK				0x41//original is 0x40
...
#define APWAKE_REQUEST 	0
#define APWAKE_NOT_REQUEST 	1
...
struct asusdec_chip {
	...
	//Shouchung end
	struct work_struct asusdec_dock_init_work;
	struct work_struct asusdec_touchpad_init_work;
	struct work_struct asusdec_hall_sensor_work;
	struct work_struct asusdec_work;
	struct work_struct asusdec_fw_update_work;
	struct work_struct asusdec_enter_s3_work;
	struct work_struct asusdec_kb_report_work;
	struct work_struct asusdec_tp_report_work;
...
struct elan_i2c_data {
	...
	//Shouchung add, for new touchpad driver
	int 				btn_left;
	int 				btn_right;
	//Shouchung end
};
