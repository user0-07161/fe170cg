...
//	struct akm_compass_data *akm = i2c_get_clientdata(client);
	...
	if(AKM_DEBUG_MESSAGE) printk("alp : get_akm9911_state 1+\n");
...
static int doeplus_state = 0;
static int dclibrary_state = 0;
static int hlayout_state = 0;
static ssize_t show_doeplus_state(struct device *dev, struct device_attribute *devattr, char *buf)
{
	return sprintf(buf, "%d\n", doeplus_state);
}

static ssize_t set_doeplus_state(struct device *dev, struct device_attribute *devattr, char *buf ,size_t count)
{
	unsigned long val;
	int ret;
...
static ssize_t show_dclibrary_state(struct device *dev, struct device_attribute *devattr, char *buf)
{
	return sprintf(buf, "%d\n", dclibrary_state);
}

static ssize_t set_dclibrary_state(struct device *dev, struct device_attribute *devattr, char *buf ,size_t count)
{
	unsigned long val;
	int ret;
	if (kstrtoul(buf, 10, &val) < 0)
...
static ssize_t show_hlayout_state(struct device *dev, struct device_attribute *devattr, char *buf)
{
	return sprintf(buf, "%d\n", hlayout_state);
}

static ssize_t set_hlayout_state(struct device *dev, struct device_attribute *devattr, char *buf ,size_t count)
{
	unsigned long val;
	int ret;
	if (kstrtoul(buf, 10, &val) < 0)
...
static DEVICE_ATTR(doeplus, 0664, show_doeplus_state, set_doeplus_state);
static DEVICE_ATTR(dclibrary, 0664, show_dclibrary_state, set_dclibrary_state);
static DEVICE_ATTR(hlayout, 0664, show_hlayout_state, set_hlayout_state);

static struct attribute *akm9911_attributes[] = {
	&dev_attr_state.attr,
	&dev_attr_rawdata.attr,
	&dev_attr_doeplus.attr,
	&dev_attr_dclibrary.attr,
	&dev_attr_hlayout.attr,
	NULL
};
...
*/
	if(Read_HW_ID() == HW_ID_PR || Read_HW_ID() == HW_ID_MP)
		pdata->layout	= 6;
	else if(Read_HW_ID() == HW_ID_ER)
		pdata->layout	= 8;
	else 
		pdata->layout	= 4;
