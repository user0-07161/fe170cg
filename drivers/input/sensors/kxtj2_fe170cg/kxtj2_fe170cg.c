...
static int asus_accel_set_cali(struct kionix_accel_driver *acceld)
{
	...
	if (isSetCali) {
		KMSGINF(&acceld->client->dev, "already set calibration value, do nothing\n");
		return 0;
	}
	...
	if (IS_ERR(fp)) {
		KMSGERR(&acceld->client->dev, "%s: open calibration file error, error = %ld\n", \
			__func__, IS_ERR(fp));
		return -1;
	}
	len = fp->f_op->read(fp, buf, 30, &fp->f_pos);
	KMSGINF(&acceld->client->dev, "%s: file length = %d, content = %s\n", __func__, len, buf);
	set_fs(old_fs);
	filp_close(fp, NULL);

	if (len <= 0) {
		KMSGERR(&acceld->client->dev, \
			"%s: cannot set calibration value because data length = %d\n", \
			__func__, len);
		return -1;
	}
	...
	if (kionix_strtok(buf, (size_t)len, buf2, cali_count) < 0)
		KMSGERR(&acceld->client->dev, "%s: do kionix_strtok error\n", __func__);
	else {
		for (i=0; i<cali_count; ++i)
			KMSGINF(&acceld->client->dev, \
				"%s: split string from calibration file, string[%d] = %s\n", \
				__func__, i, buf2[i]);
		...
		for (i=0; i<cali_count; ++i)
			KMSGINF(&acceld->client->dev, \
				"%s: convert calibration string to integer, integer[%d] = %ld\n", \
...
static int asus_accel_grp4_clear_interrupt(struct kionix_accel_driver *acceld)
{
	...
	KMSGINF(&acceld->client->dev, "%s: clear interrupt\n", __func__);
	...
	if (err < 0)
		KMSGERR(&acceld->client->dev, "%s: clear interrupt error, error = %d\n", \
			__func__, err);
				...
				struct device_attribute *devattr, char *buf)
{
	...
	KMSGINF(&acceld->client->dev, "%s: the WHO_AM_I return value = %d\n", \
		__func__, kxtj2_wia);
				 ...
				 const struct i2c_device_id *id)
{
	...
	KMSGINF(&client->dev, "%s: enter probe function\n", __func__);
	...
	KMSGINF(&client->dev, "%s: i2c bus address = %x\n", __func__, acceld->client->addr);
	...
	KMSGINF(&client->dev, "%s: the WHO_AM_I return value = %d\n", __func__, err);
	...
	if (acceld->accel_drdy) {
		KMSGINF(&client->dev, "%s: int_irq = %d, int_gpio = %d\n", \
			__func__, int_irq, accel_pdata->int_gpio);
	...
	KMSGINF(&client->dev, "%s: probe successfully\n", __func__);
