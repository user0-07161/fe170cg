...
#include <linux/input/kxtj2_tf103.h>
				 ...
				 const struct i2c_device_id *id)
{
	...
	if (KXTJ2_DEBUG)
		printk("KXTJ2_DEBUG : i2c_check_functionality.\n");
	...
	if (KXTJ2_DEBUG)
		printk("KXTJ2_DEBUG : accel_pdata.\n");
	...
	if (KXTJ2_DEBUG)
		printk("KXTJ2_DEBUG : kzalloc.\n");

	acceld->client = client;
	acceld->accel_pdata = *accel_pdata;

	if (KXTJ2_DEBUG)
		printk("KXTJ2_DEBUG : addr:%d\n", acceld->client->addr);
	...
	if (KXTJ2_DEBUG)
		printk("KXTJ2_DEBUG : kionix_verify:%d.\n", err);
	...
	kionix_accel_update_direction(acceld);
   //+++ Panda mark this line for build error +++
   /*
   ...
   //--- Panda mark this line for build error ---
	...
	if (acceld->accel_drdy) {
		if (KXTJ2_DEBUG) {
			printk("KXTJ2_DEBUG : int_irq = %d.\n", int_irq);
			printk("KXTJ2_DEBUG : int_gpio = %d.\n", accel_pdata->int_gpio);
		}
...
static struct i2c_driver kionix_accel_driver_tf103 = {
	.driver = {
...
{	
	//[Workaround]We pull up gpio 36 for lcm brige
	//If we do not pull up this pin ,the i2c bus 
	//will block by lcm bridge 	
	int err	= 0;
	err = gpio_request(36, "lcm_brige");
		if (err) {
			printk("request gpio lcm_brige failed.\n");
			return err;
		}
		gpio_direction_output(36, 1);

		if (KXTJ2_DEBUG)
			printk("KXTJ2_DEBUG : gpio pin 36 pull high .\n");
	
	
	return i2c_add_driver(&kionix_accel_driver_tf103);
}
module_init(kionix_accel_init);

static void __exit kionix_accel_exit(void)
{
	i2c_del_driver(&kionix_accel_driver_tf103);
}
