...
//	struct akm_compass_data *akm = file->private_data;
	...
	case ECS_IOCTL_GET_OPEN_STATUS:
		...
		printk("+++Compass Driver:IOCTL_GET_OPEN_STATUS called.+++\n");
	...
	case ECS_IOCTL_GET_CLOSE_STATUS:
		...
		printk("+++Compass Driver:IOCTL_GET_CLOSE_STATUS called.+++\n");
...
static int akm_compass_suspend_tf103(struct device *dev)
{
//	struct akm_compass_data *akm = dev_get_drvdata(dev);
	dev_dbg(&s_akm->i2c->dev, "suspended\n");
	printk("TF103 alp : akm_compass_suspend !! \n");
	return 0;
}

static int akm_compass_resume_tf103(struct device *dev)
{
//	struct akm_compass_data *akm = dev_get_drvdata(dev);
	dev_dbg(&s_akm->i2c->dev, "resumed\n");
	printk("TF103 alp : akm_compass_resume !! \n");
	return 0;
}
...
#endif
int akm_compass_probe_tf103(struct i2c_client *client, const struct i2c_device_id *id)
{
...
//	dev_dbg(&client->dev, "start probing.");
	printk("TF103 akm_compass_probe ++\n");
...
*/
	//printk("alp : akm09911 gpio_RSTN =%d \n",pdata->gpio_RSTN);
	s_akm->gpio_rstn=0;
	s_akm->layout = 5;
/*	
#if CONFIG_ME372CG
	s_akm->gpio_rstn = pdata->gpio_RSTN; // use 79
#endif

#if CONFIG_ME372CL
	s_akm->gpio_rstn = 0; // not ready , need GPIO table
#endif
*/
	/***** I2C initialization *****/
	s_akm->i2c = client;
	...
	if (err) {
		dev_err(&client->dev,
			"%s: TF103 input_dev register failed", __func__);
		goto exit3;
	}
...
//	s_akm->irq = client->irq;
	...
	dev_dbg(&client->dev, "%s: IRQ is #%d.", __func__, s_akm->irq);
	...
	if (s_akm->irq) {
		...
		printk("alp : did not use irq , daemon control!! \n");
	}
	...
	if (err) {
		dev_err(&client->dev,
			"%s: TF103 akm_compass_dev register failed", __func__);
		goto exit5;
	}
	...
	if (0 > err) {
		dev_err(&client->dev,
			"%s: TF103 create sysfs failed.", __func__);
		goto exit6;
	}
	...
	if (err) {
		if(AKM_DEBUG_MESSAGE) printk("TF103 alp : sysfs_create_group fail\n");
	}
#endif
//	dev_info(&client->dev, "successfully probed.");
	printk("TF103 alp akm_compass_probe --\n");
	return 0;
...
static int akm_compass_remove_tf103(struct i2c_client *client)
{
	struct akm_compass_data *akm = i2c_get_clientdata(client);
...
static const struct dev_pm_ops akm_compass_pm_ops_tf103 = {
	.suspend	= akm_compass_suspend_tf103,
	.resume		= akm_compass_resume_tf103,
};

static struct i2c_driver akm_compass_driver = {
	.probe		= akm_compass_probe_tf103,
	.remove		= akm_compass_remove_tf103,
	.id_table	= akm_compass_id,
	.driver = {
		.name	= AKM_I2C_NAME,
		.pm		= &akm_compass_pm_ops_tf103,
	},
};

static int __init akm_compass_init_tf103(void)
{
	pr_info("TF103 AKM compass driver: initialize.");
	return i2c_add_driver(&akm_compass_driver);
}

static void __exit akm_compass_exit_tf103(void)
{
	pr_info("TF103 AKM compass driver: release.");
	i2c_del_driver(&akm_compass_driver);
}

module_init(akm_compass_init_tf103);
module_exit(akm_compass_exit_tf103);
