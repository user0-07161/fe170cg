...
extern int read_SIM_ID(void);
extern void initialize_VOICE_ID(void);
#endif
...
{
#ifdef CONFIG_FE170CG
    /*This line should be move to another appropriate driver code position*/
    read_SIM_ID();
#endif
...
#ifdef CONFIG_FE170CG	
	// Chang VNN to 1.05V only used for test.
	// VNN to 1.05V
	ret = intel_scu_ipc_iowrite8(0xc1, 0xA4);
	dev_info(&rpdev->dev, "Chang VNN to 1.05\n");
	if (ret) {
	    dev_err(&rpdev->dev, "Chang VNN to 1.05V fail\n");
	}
    initialize_VOICE_ID();
#endif
	goto out;
