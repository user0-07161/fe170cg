				     ...
				     const struct pci_device_id *ent)
{
	...
	for (i = 0; i < slots; i++) {
		...
		chip->slots[i] = slot;
		if (gpio_is_valid(slot->cd_gpio) && gpio_get_value(slot->cd_gpio)) {
			/* card is absent */
			vmmc = slot->host->vmmc;
			if (vmmc != NULL && regulator_is_enabled(vmmc)) {
				dev_err(&pdev->dev, "Card is absent but power still enabled.\n");
				regulator_force_disable(vmmc);
			}
		}
	}
