...
void mmc_detect_change(struct mmc_host *host, unsigned long delay)
{
	...
	//nick_hsiao:replace flush_work_sync by cancel_delayed_work for debouncing
	//and avoiding "bad: scheduling from the idle thread"
...
void mmc_rescan(struct work_struct *work)
{
 ...
 out:
	...
	if (host->caps & MMC_CAP_NEEDS_POLL) {
		 ...
		 */
		if(host->retry_timeout > 10) host->retry_timeout = 10;
