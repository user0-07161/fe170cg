...
#define DEF_GYRO_WAIT_TIME          20//50
...
static int inv_check_accl_self_test_raw(struct inv_mpu_iio_s *st, int *reg_avg)
{
#define ACCEL_THRESHOLD_MG  DEF_ACCEL_ST_SHIFT_MAX
    ...
    accel_bias[Z] = abs(abs(reg_avg[Z]*st->chip_info.multi) - (1L<<15)/(ACCL_G_SCALE << DEF_SELFTEST_ACCL_FULL_SCALE)*DEF_GYRO_PRECISION);
    thres = ACCEL_THRESHOLD_MG*(1L << 15)/(ACCL_G_SCALE << DEF_SELFTEST_ACCL_FULL_SCALE);
    printk("accel_bias=%d %d %d, thres=%ld\n", accel_bias[X], accel_bias[Y], accel_bias[Z], thres);
    for (j = 0; j < 3; j++) {
        if (accel_bias[j] > thres) {
            ret_val |= (1<<j);
            printk("%s, axis(%d) > ACCEL_THRESHOLD_MG\n", __func__, j);
        }
    }
...
//                        compass_result = !inv_check_compass_self_test(st);
       ...
       //accel_result = inv_check_accl_self_test(st, accl_bias_regular, accl_bias_st);
        accel_result |= inv_check_accl_self_test_raw(st, accl_bias_regular);
        accel_result = !accel_result;
        //gyro_result = inv_check_6050_gyro_self_test(st, gyro_bias_regular, gyro_bias_st);
        gyro_result |= inv_check_gyro_self_test_raw(st, gyro_bias_regular);
        gyro_result = !gyro_result;
test_fail:
        inv_recover_setting(st);
        return (accel_result<<DEF_ST_ACCEL_RESULT_SHIFT) | gyro_result ; //| (compass_result<<DEF_ST_COMPASS_RESULT_SHIFT) ;
