...
#define MAX_FIFO_I2C_TIMEOUT 5
static int mpu_fifo_i2c_timeout_counter = 0;
static int is_irq_stopped = 0;
//------
...
static int reset_fifo_itg(struct iio_dev *indio_dev)
{
	...
	if (result) {
		printk("int_enable write failed\n");
		return result;
	}
	/* disable the sensor output to FIFO */
	result = inv_i2c_single_write(st, reg->fifo_en, 0);
...
irqreturn_t inv_read_fifo(int irq, void *dev_id)
{
	...
	if (st->chip_config.lpa_mode) {
		...
		//+++++++
		if(result == -ETIMEDOUT){
			mpu_fifo_i2c_timeout_counter++;
		}else{
			mpu_fifo_i2c_timeout_counter = 0;
		}
		//-------
		if (result)
			goto end_session;
...
end_session:
	...
	//++++++
	if(mpu_fifo_i2c_timeout_counter >= MAX_FIFO_I2C_TIMEOUT){
		printk("mpu6500 error : free irq because i2c timeout err\n");
		is_irq_stopped = 1;
...
//++++++
int inv_mpu_is_irq_stopped(void){
	return is_irq_stopped;
}
//------
/**
 *  @}
