	...
	struct device_attribute *attr, char *buf)
{
	...
	if (result) {
		printk("Could not read temperature register.\n");
		return result;
	}
...
{	
	...
	buffer[0] = REG_WHOAMI;
	err = mpu6500_i2c_rxdata(client, &(buffer[0]), 1);
	if(err < 0)
		printk("get_akm8963_state wia : %d\n",err);
	printk("get_mpu6500_state wia : %d\n",buffer[0]);
//	if(buffer[0]==0x68) || (buffer[0]==0x70)
	if( (buffer[0]==104) || (buffer[0]==112) )
		mpu6500_wia =  1;
	else
		mpu6500_wia =  0;
	return sprintf(buf, "%d\n",mpu6500_wia);
...
static ssize_t get_mpu6500_gyro_rawdata(struct device *dev, struct device_attribute *devattr, char *buf)
{	
	struct i2c_client *client = to_i2c_client(dev);
	uint8_t buffer[7];
	int err=0;
	...
	err = mpu6500_i2c_rxdata(client, &(buffer[0]), 1);
	printk("get_mpu6500_gyro_rawdata (%d) +\n",buffer[0]);
	buffer[1] = (buffer[0] | 7);
	buffer[6] = buffer[0];
	buffer[0] = REG_PWR_MGMT_2;
	printk("get_mpu6500_gyro_rawdata (%d) -\n",buffer[0]);
	err = mpu6500_i2c_txdata(client, &(buffer[0]), 2);

	buffer[0] = REG_RAW_GYRO;
	err = mpu6500_i2c_rxdata(client, &(buffer[0]), 6);
	if (err < 0){
		printk("get_mpu6500_gyro_rawdata fail\n");
		return err;
	}
	printk("Gyro rawdataX : (%d) | (%d)\n",buffer[0], buffer[1]);
	printk("Gyro rawdataY : (%d) | (%d)\n",buffer[2], buffer[3]);
	printk("Gyro rawdataZ : (%d) | (%d)\n",buffer[4], buffer[5]);
...
static ssize_t get_mpu6500_gsensor_rawdata(struct device *dev, struct device_attribute *devattr, char *buf)
{	
	struct i2c_client *client = to_i2c_client(dev);
	uint8_t buffer[6];
	int err=0;
	...
	err = mpu6500_i2c_rxdata(client, &(buffer[0]), 6);
	if (err < 0){
		printk("get_mpu6500_gsensor_rawdata fail\n");
		return err;
	}
	...
	if(rawdata_z>0x7FFF)
		rawdata_z = rawdata_z-0x10000;
	printk("Accel Rawdata : (%d), (%d), (%d)\n",rawdata_x, rawdata_y, rawdata_z);
	return sprintf(buf, "%d , %d , %d\n",rawdata_x, rawdata_y, rawdata_z);
}

//+++++++
static ssize_t get_mpu6500_is_irq_stopped(struct device *dev, struct device_attribute *devattr, char *buf)
{
	return sprintf(buf, "%d\n",inv_mpu_is_irq_stopped());
}
//-------
...
//++++++
static DEVICE_ATTR(is_irq_status_stop, S_IRUGO, get_mpu6500_is_irq_stopped, NULL);
//------
...
//++++++
	&dev_attr_is_irq_status_stop.attr,
//------
	NULL
};

static struct attribute_group i2c_mpu6500_attributes_group = {
	.attrs = i2c_mpu6500_attributes
};
	...
	const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		result = -ENOSYS;
		printk("alp : I2c function error\n");
		printk("I2c function error\n");
		goto out_no_free;
	}
	indio_dev = iio_allocate_device(sizeof(*st));
	if (indio_dev == NULL) {
		printk("alp : memory allocation failed\n");
	...
	result = gpio_request(pdata->int_gpio,"mpu6500_int");
	if (result < 0)
		printk("alp : G-sensor gpio_request fail err =%d \n",result);
	result = gpio_direction_input(pdata->int_gpio);
	if (result < 0)
		printk("alp : G-sensor gpio_direction_input fail err =%d \n",result);
	st->client->irq = gpio_to_irq(pdata->int_gpio);
	printk("alp : client irq(%d)\n",st->client->irq);

	result = inv_check_chip_type(st, id);
	if (result){
		printk("alp : inv_check_chip_type failed!!!!\n");
		goto out_free;
	}

	result = st->init_config(indio_dev);
	if (result) {
		printk("alp : init_config failed!!!!\n");
	...
	result = st->set_power_state(st, false);
	if (result) {
		printk("alp : set_power_state failed!!!!\n");
		dev_err(&client->adapter->dev,
	...
	if (result) {
		printk("alp : inv_mpu_configure_ring failed!!!!\n");
		printk("configure ring buffer fail\n");
		goto out_free;
	}
	result = iio_buffer_register(indio_dev, indio_dev->channels,
					indio_dev->num_channels);
	if (result) {
		printk("alp : iio_buffer_register failed!!!!\n");
		printk("ring buffer register fail\n");
		goto out_unreg_ring;
	}
	st->irq = client->irq;
	result = inv_mpu_probe_trigger(indio_dev);
	if (result) {
		printk("alp : inv_mpu_probe_trigger failed!!!!\n");
		printk("trigger probe fail\n");
	...
	if (result) {
		printk("alp : iio_device_register failed!!!!\n");
		printk("IIO device register fail\n");
	...
	if (INV_MPU6050 == st->chip_type ||
		...
		if (result) {
			printk("alp : inv_create_dmp_sysfs failed!!!!\n");
			printk("create dmp sysfs failed\n");
			goto out_unreg_iio;
		}
	}
	...
	result = sysfs_create_group(&client->dev.kobj,&i2c_mpu6500_attributes_group);
	if (result) {
...
{
	printk("alp : inv_mod_init ++(%d)\n");
	int result = i2c_add_driver(&inv_mpu_driver);
	if (result) {
		printk("alp : inv_mod_init faild\n");
		printk("failed\n");
		return result;
	}
	return 0;
