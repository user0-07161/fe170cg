...
#if defined(CONFIG_FE170CG) || defined(CONFIG_ME560CG) || defined(CONFIG_TF103CG)
...
void penwell_update_transceiver(void)
{
	...
	if (!pnw->queue_stop) {
		if(iotg->otg.state == OTG_STATE_B_IDLE)
			queue_delayed_work(pnw->qwork, &pnw->work, OTG_DELAY_TIME);
		else
			queue_delayed_work(pnw->qwork, &pnw->work, 0);
...
#if defined(CONFIG_FE170CG) || defined(CONFIG_ME560CG) || defined(CONFIG_TF103CG)
	printk(KERN_INFO "%s setSMB347Charger +++\n", __func__);
...
static int penwell_otg_charger_det_clt(void)
{
	...
	if (retval < 0) {
		...
		dev_dbg(pnw->dev, "CDP detected\n");
		penwell_otg_charger_hwdet(true);
		return CHRG_CDP;
	} else  if (retval == CHRG_DCP) {
		dev_dbg(pnw->dev, "DCP detected\n");
...
void penwell_otg_phy_power(int on)
{
	...
	if (is_clovertrail(to_pci_dev(pnw->dev))) {
		dev_info(pnw->dev, "turn %s USB PHY by gpio_cs(%d)\n",
					on ? "on" : "off",
...
static void penwell_otg_eye_diagram_optimize(void)
{
	...
	retval = penwell_otg_ulpi_read(iotg, ULPI_VS1, &data);
	printk(KERN_INFO "%s USB strength %#X\n", __func__, data);
}
					...
					struct intel_mid_otg_xceiv *iotg)
{
	...
	if (int_mask) {
		...
		if (int_mask & OTGSC_IDIS) {
			if (!id)
				iotg->hsm.id = (int_sts & OTGSC_ID) ?
					ID_B : ID_A;
			flag = 1;
			dev_info(pnw->dev, "%s: id change int = %d\n",
						__func__, iotg->hsm.id);
		...
		if (int_mask & OTGSC_DPIS) {
			...
			flag = 1;
			dev_info(pnw->dev, "%s: data pulse int = %d\n",
						__func__, iotg->hsm.a_srp_det);
		}
		if (int_mask & OTGSC_BSEIS) {
			iotg->hsm.b_sess_end = (int_sts & OTGSC_BSE) ? 1 : 0;
			flag = 1;
			dev_info(pnw->dev, "%s: b sess end int = %d\n",
						__func__, iotg->hsm.b_sess_end);
		}
		if (int_mask & OTGSC_BSVIS) {
			iotg->hsm.b_sess_vld = (int_sts & OTGSC_BSV) ? 1 : 0;
			flag = 1;
			dev_info(pnw->dev, "%s: b sess valid int = %d\n",
						__func__, iotg->hsm.b_sess_vld);
		}
		if (int_mask & OTGSC_ASVIS) {
			iotg->hsm.a_sess_vld = (int_sts & OTGSC_ASV) ? 1 : 0;
			flag = 1;
			dev_info(pnw->dev, "%s: a sess valid int = %d\n",
						__func__, iotg->hsm.a_sess_vld);
		}
		if (int_mask & OTGSC_AVVIS) {
			iotg->hsm.a_vbus_vld = (int_sts & OTGSC_AVV) ? 1 : 0;
			flag = 1;
			dev_info(pnw->dev, "%s: a vbus valid int = %d\n",
...
static int penwell_otg_suspend(struct device *dev)
{
	...
	if (delayed_work_pending(&pnw->work)) {
		dev_info(pnw->dev, "pnw->work still pending, don't suspend\n");
		spin_lock(&pnw->lock);
		pnw->queue_stop = 0;
		...
		flush_delayed_work_sync(&pnw->work);
