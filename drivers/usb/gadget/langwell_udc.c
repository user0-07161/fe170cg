...
static ssize_t
show_usb_speed_switch(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct langwell_udc *udc = the_controller;
	...
	next = buf;
	size = PAGE_SIZE;
	devlc = readl(&udc->op_regs->devlc);

	t = scnprintf(next, size, "%s\n",(devlc & LPM_PFSC) ? "USB1.X" : "USB2.0");
...
static ssize_t store_usb_speed_switch(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(buf){
		...
		intel_mid_stop_peripheral(NULL);
	}
	intel_mid_start_peripheral(NULL);
	return count;
}
static DEVICE_ATTR(speed_switch, S_IRUGO | S_IWUSR | S_IWGRP, show_usb_speed_switch, store_usb_speed_switch);
...
static void langwell_udc_remove(struct pci_dev *pdev)
{
	...
	device_remove_file(&pdev->dev, &dev_attr_speed_switch);
		...
		const struct pci_device_id *id)
{
	...
	retval = device_create_file(&pdev->dev, &dev_attr_speed_switch);
	if (retval)
		goto error_attr4;
