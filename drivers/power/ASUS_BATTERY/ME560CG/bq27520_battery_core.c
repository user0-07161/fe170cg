...
static void checkBatteryInvalidStatus(struct device *dev) {
    int   temperature;
    int   capacity;
    ...
    if (batt_info.cable_status == USB_ADAPTER) {
        ...
        if (temperature >= 500 || temperature <= 0) {
             ...
             dev_info(dev, "=== battery is in normal temperatue with AC charger during sleep ======, temperature = %d\n", temperature);
...
static int bq27520_bat_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    int ret = 0;
    int chemicalID = 0;
