...
static int bq27520_proc_forceupdate_read(struct seq_file *m, void *p) {
    int len;

    len = seq_printf(m,"update status: %d\n", ret);
    return len;
}

static ssize_t bq27520_proc_forceupdate_write(struct file *file, const char *buffer, size_t count, loff_t *data) {
    ret = UPDATE_NONE;
...
static int bq27520_proc_enter_rommode_read(struct seq_file *m, void *p) {
    int len;
...
static ssize_t bq27520_proc_enter_rommode_write(struct file *file, const char *buffer, size_t count, loff_t *data) {
    ret = -1;
...
static int bq27520_proc_update_from_dffs_read(struct seq_file *m, void *p) {
    int len;
...
static ssize_t bq27520_proc_update_from_dffs_write(struct file *file, const char *buffer, size_t len, loff_t *data) {
...
//bq27520_gauge_forceupdate      ------START
static int bq27520_gauge_forceupdate_open(struct inode *inode, struct file *file) {
	return single_open(file, bq27520_proc_forceupdate_read, NULL);
}

static const struct file_operations bq27520_gauge_forceupdate_ops = {
	.open		= bq27520_gauge_forceupdate_open,
	.read		= seq_read,
        .write          = bq27520_proc_forceupdate_write,
	.llseek		= seq_lseek,
	.release	= seq_release
};
//bq27520_gauge_forceupdate       ------END
//bq27520_gauge_enter_rom_mode      ------START
static int bq27520_gauge_enter_rom_mode_open(struct inode *inode, struct file *file) {
	return single_open(file, bq27520_proc_enter_rommode_read, NULL);
}

static const struct file_operations bq27520_gauge_enter_rom_mode_ops = {
	.open		= bq27520_gauge_enter_rom_mode_open,
	.read		= seq_read,
        .write          = bq27520_proc_enter_rommode_write,
	.llseek		= seq_lseek,
	.release	= seq_release
};
//bq27520_gauge_enter_rom_mode       ------END
//bq27520_gauge_update_from_dffs     ------START
static int bq27520_gauge_update_from_dffs_open(struct inode *inode, struct file *file) {
	return single_open(file, bq27520_proc_update_from_dffs_read, NULL);
}

static const struct file_operations bq27520_gauge_update_from_dffs_ops = {
	.open		= bq27520_gauge_update_from_dffs_open,
	.read		= seq_read,
        .write          = bq27520_proc_update_from_dffs_write,
	.llseek		= seq_lseek,
	.release	= seq_release
};
//bq27520_gauge_update_from_dffs      ------END

int bq27520_register_gauge_forceupdate_proc_fs(void) {
        struct proc_dir_entry *entry=NULL;

        entry = proc_create("bq27520_gauge_forceupdate", 0666, NULL, &bq27520_gauge_forceupdate_ops);
        if (!entry) {
            printk("[%s]Unable to create bq27520 gauge forceupdate \n", __FUNCTION__);
            return -EINVAL;
        }

        entry = proc_create("bq27520_gauge_enter_rom_mode", 0664, NULL, &bq27520_gauge_enter_rom_mode_ops);
        if (!entry) {
            printk("[%s]Unable to create bq27520 gauge enter rom mode \n", __FUNCTION__);
            return -EINVAL;
        }

        entry = proc_create("bq27520_gauge_update_from_dffs", 0664, NULL, &bq27520_gauge_update_from_dffs_ops);
        if (!entry) {
