...
// check gauge vesion is G3 or G4
    if (strncmp(batt_info.serial_number, BQ27520_G3_VERSION, 4) == 0
       || strncmp(batt_info.serial_number, BQ27520_G4_VERSION, 4) == 0) {
	   if (batt_info.gauge_version == IC_VERSION_G3) {
        ...
        BAT_DBG_E("bq27520 is not G3 or G4!!!!\n");
        ret = UPDATE_GAUGE_VERSION;
        goto Done;
...
int bq27520_bat_upt_main_update_flow(bool forceupdate)
{
    ...
    if (bq27520_is_rom_mode()) {
        ...
        flash_gauge_status = UPDATE_CHECK_MODE_FAIL; 
...
static int bq27520_bat_upt_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id) {
