...
int thermal_charging_toggle_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
...
static const struct file_operations thermal_charging_limit_ops = {
	.read		= seq_read,
...
int init_thermal_charging_toggle(void)
{
    ...
    entry = proc_create("thermal_charging_limit", 0600, NULL, &thermal_charging_limit_ops);
    if (!entry) {
...
{
        int gauge_percentage = -EINVAL;
        int user_percentage  = 50;
        drv_status_t drv_status;
        struct battery_info_reply tmp_batt_info;
        ...
        if (tmp_batt_info.tbl && tmp_batt_info.tbl->read_percentage) {
                gauge_percentage = tmp_batt_info.tbl->read_percentage();

                if (gauge_percentage == ERROR_CODE_I2C_FAILURE)
                    return -EINVAL;

                if (gauge_percentage < 5) {
                        user_percentage = 0;
                        drv_status = DRV_SHUTDOWN;
                } else if (gauge_percentage >= 5 && gauge_percentage <= 30) {
                        user_percentage = gauge_percentage - 5;
                } else if (gauge_percentage >= 31 && gauge_percentage <= 50) {
                        user_percentage = gauge_percentage - 4;
                } else if (gauge_percentage >= 51 && gauge_percentage <= 60) {
                        user_percentage = gauge_percentage - 3;
                } else if (gauge_percentage >= 61 && gauge_percentage <= 70) {
                        user_percentage = gauge_percentage - 2;
                } else if (gauge_percentage >= 71 && gauge_percentage <= 80) {
                        user_percentage = gauge_percentage - 1;
                } else {
                        user_percentage = gauge_percentage;
                }
                batt_info.drv_status = drv_status;
        }
        printk(KERN_WARNING "<BATT> Gauge P:%d %%,User P:%d %%)\n",
                gauge_percentage,
                user_percentage);

        return user_percentage; //% return adjust percentage, and error code ( < 0)
}
...
static int asus_battery_update_status_no_mutex(int percentage)
{
        ...
        if (cable_status == USB_ADAPTER || cable_status == USB_PC) {
			...
			/* ME560CG limit to protect battery from damaged when battery temperature is too low or High*/
			temperature = asus_battery_update_temp_no_mutex();
                        voltage = asus_battery_update_voltage_no_mutex();
                        if (temperature != ERROR_CODE_I2C_FAILURE) {
                                if (temperature <= 0) {
					smb347_set_voltage(4320);
                                        smb347_charging_toggle(false);
                                        status = POWER_SUPPLY_STATUS_DISCHARGING;
                                        goto final;
