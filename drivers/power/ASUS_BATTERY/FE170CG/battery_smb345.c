...
static int battery_temp_zone = TEMP_0_50;
int battery_ID = 0;//0:FE170CG ,1:FE70CG
EXPORT_SYMBOL(battery_ID);
#endif
...
int setSMB345Charger(int usb_state) {
    ...
    case USB_IN:
        ...
        if (smb345_dev && !gpio_get_value(smb345_dev->pdata->inok_gpio)) {
            ...
            /* FE170CG charge current control algorithm:
...
int do_battery_jeita(int batt_tempr, int temp_zone_last) {
   ...
   int ret, need_add_0 = 0, need_add_50_55 = 0, need_add_55 = 0;
   ...
   if (batt_tempr < (0 + need_add_0)) {
      if (battery_ID == 1) {
          //BAT_DBG(" %s:============= BATT ID = 1 , set 4.30 V !!! \n", __func__);
          /* Vchg=4.30v , write 03h[5:0]="101000"*/
          ret = smb345_masked_write(smb345_dev->client,
            FLOAT_VOLTAGE_REG,
            BIT(0) | BIT(1) |BIT(2) |BIT(3) |BIT(4) |BIT(5),
   ...
   } else if (batt_tempr >= (0 + need_add_0) && batt_tempr < (500 - need_add_50_55)) {
      if (battery_ID == 1) {
          //BAT_DBG(" %s:============= BATT ID = 1 , set 4.30 V !!! \n", __func__);
          /* Vchg=4.30v , write 03h[5:0]="101000"*/
          ret = smb345_masked_write(smb345_dev->client,
      ...
      /* Charging Enable */
      ret = smb345_charging_toggle(JEITA, true);
      /* Recharge voltage  =4.15v , write 01h[3:2]="10"*/
      ret = smb345_masked_write(smb345_dev->client,
            0x01,
            BIT(3)|BIT(2),
            BIT(3));
      ...
      /* Vchg=4.11v , write 03h[5:0]="011110"*/
            ret = smb345_masked_write(smb345_dev->client,
                FLOAT_VOLTAGE_REG,
                FLOAT_VOLTAGE_MASK,
                0x1E);
      /* Charging Enable */
      ret = smb345_charging_toggle(JEITA, true);
      /* Recharge voltage  =4.01v , write 01h[3:2]="01"*/
      ret = smb345_masked_write(smb345_dev->client,
            0x01,
            BIT(3)|BIT(2),
            BIT(2));
...
#ifdef CONFIG_FE170CG
int Read_BATT_ID(void) {

    int batt_id = get_gpio_by_name("BAT_ID_W");
    if (batt_id == -1) {
        printk(">>> fail to get BAT_ID_W !!! <<<\n");
        return battery_ID;
    }

    if (gpio_get_value(batt_id)) {
        printk(">>> BATT_ID (HIGH) <<<\n");
        battery_ID = 0;
    } else {
        printk(">>> BATT_ID (LOW) <<<\n");
...
EXPORT_SYMBOL(Read_BATT_ID);

static int batt_id_read(struct seq_file *m, void *p) {
     int len = 0;
...
static int proc_batt_id_open(struct inode *inode, struct file *file) {
	return single_open(file, batt_id_read, NULL);
}

static const struct file_operations proc_batt_id_ops = {
	.open		= proc_batt_id_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= seq_release
};

int register_battery_id_proc_fs(void) {
        struct proc_dir_entry *entry = NULL;

        entry = proc_create("battery_ID", 0664, NULL, &proc_batt_id_ops);
        if (!entry) {
            printk("[%s]Unable to create battery_ID \n", __FUNCTION__);
            return -EINVAL;
        }
        return 0;
}
...
static int smb345_probe(struct i2c_client *client, const struct i2c_device_id *id) {
    ...
    ret = register_battery_id_proc_fs();
    if (ret) {
        BAT_DBG_E("Unable to register_battery_id_proc_fs\n");
    }
#endif
