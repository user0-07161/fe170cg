...
#include "ug31xx_ggb_data_FE170CG_20140516.h"
#include "ug31xx_ggb_data_FE70CG_20140519.h"
#include "../uPILogger.h"
#endif
...
bool ug31xx_probe_done = false;
EXPORT_SYMBOL(ug31xx_probe_done);
extern int Read_HW_ID(void);
extern int battery_ID;
...
/* Extern Function */
extern char FactoryGGBXFile_FE170CG[];
extern char FactoryGGBXFile_FE70CG[];
...
{
        if (battery_ID == 0)
	    FactoryGGBXFile = FactoryGGBXFile_FE170CG;
        else 
            FactoryGGBXFile = FactoryGGBXFile_FE70CG;
}
			     ...
			     union power_supply_propval *val)
{
	...
	if(psp == POWER_SUPPLY_PROP_CURRENT_NOW)
	{
		...
		mutex_unlock(&ug31->info_update_lock);
		if(( cur_cable_status == UG31XX_NO_CABLE ) && (val->intval > 0)) // 2014/04/07 Tom
		{
			val->intval = 0;
		}
		val->intval = val->intval * 1000;
...
static void batt_info_update_work_func(struct work_struct *work)
{
  ...
  if(cos_file_exist == true)
  {
    schedule_delayed_work(&ug31->cos_check_work, UG31XX_CALI_BO_FACTORY_DELAY*HZ);
  }
}
...
static void cos_work_func(struct work_struct *work)
{
  ...
  schedule_delayed_work(&ug31->cos_work, UG31XX_CALI_BO_FACTORY_DELAY*HZ);
}
		...
		cos_file_exist == false)
	{
		...
		while(1)
		{
			...
			if(board_offset_cali_loop >= board_offset_cali_loop_max)
			{
				...
				cos_result = ug31_module.get_board_offset();
				schedule_delayed_work(&ug31->cos_work, 0*HZ);
				break;
			}
...
static int __init ug31xx_i2c_init(void)
{
	...
	//hwid = Read_HW_ID();
   hwid = 1;// alway hwid = 1 avoid MP and SR conflit
	pr_info("<BATT> ++++++++++++++++ %s ++++++++++++++++, hwid =%d\n", __func__, hwid);
