...
static int check_all_thermal_sensors(struct thermal_zone_device *tzd,unsigned long *temp)
{
	if(tzd_0 && tzd_1){
		...
		printk("thermal check_all_thermal_sensors #1\n");
...
static int mid_thermal_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < platforminfo->num_sensors; i++) {
		//ATD thermal check >>>
		if(i<2){
			printk("to init check_thermal :%d\n",i);
			tzd_ops.check_thermal = check_all_thermal_sensors;
		}
		//ATD thermal check <<<
		...
		//ATD thermal check >>>
		if(i<2){
			printk("to init tzd_0/1 :%d\n",i);
			if(i==0) tzd_0 = platforminfo->tzd[i];
			if(i==1) tzd_1 = platforminfo->tzd[i];
		}
		//ATD thermal check <<<
	}
