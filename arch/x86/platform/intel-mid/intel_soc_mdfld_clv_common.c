...
static void log_wakeup_source(int source)
{
	...
	case PMU_USB_OTG_LSS_06:
		//pr_info("wakeup from USB.\n");
                pr_info("Resume caused by WAKE%d, USB\n", PMU_USB_OTG_LSS_06);
		break;
	case PMU_GPIO0_LSS_39:
		//pr_info("wakeup from GPIO.\n");
                if (wakeup_gpio_num == 2) {
                    pr_info("Resume caused by WAKE32, wifi GPIO[%d]\n", wakeup_gpio_num);
                } else {
                    pr_info("Resume caused by WAKE%d, by GPIO\n", wakeup_gpio_num);
                }
		break;
	case PMU_HSI_LSS_03:
		//pr_info("wakeup from HSI.\n");
                pr_info("Resume caused by WAKE%02d, HSI\n", PMU_HSI_LSS_03);
		break;
	default:
		//pr_info("wakeup from LSS%02d.\n",
		//	source - mid_pmu_cxt->pmu1_max_devs);
                if ((source - mid_pmu_cxt->pmu1_max_devs) == PMU_vRTC_LSS_31) {
                   pr_info("Resume caused by WAKE18, alarm\n");
                } else {
                   pr_info("Resume caused by WAKE%02d, \n", source - mid_pmu_cxt->pmu1_max_devs);
                }
		break;
	}
}
