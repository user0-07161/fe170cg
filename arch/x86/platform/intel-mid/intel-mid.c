...
void intel_mid_set_project_id(int value){project_id = value;}
module_param(project_id, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(PROJ_VERSION,
                 "PROJ_ID judgement");

static int hardware_id;
void intel_mid_set_hardware_id(int value){hardware_id = value;}
...
static int pcb_id;
void intel_mid_set_pcb_id(int value){pcb_id = value;}
module_param(pcb_id, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(PCB_VERSION,
                 "PCB_ID judgement");

static int rc_version;
void intel_mid_set_rc_version(int value){rc_version = value;}
module_param(rc_version, int, S_IRUGO | S_IWUSR);

static int voice_id;
void intel_mid_set_voice_id(int value){voice_id = value;}
module_param(voice_id, int, S_IRUGO | S_IWUSR);

static int sim_id = 0;
void intel_mid_set_sim_id(int value){sim_id = value;}
module_param(sim_id, int, S_IRUGO | S_IWUSR);

static int rf_id;
void intel_mid_set_rf_id(int value){rf_id = value;}
