...
/* RHB OEMB table */
static struct sfi_table_oemb rhb_oemb_table = {
   .spid = {
       .customer_id = CUSTOMER_INTEL,
       .vendor_id = VENDOR_INTEL,
       .platform_family_id = INTEL_CLVTP_PHONE,
       .product_line_id = INTEL_CLVTP_PHONE_RHB_ENG,
       .hardware_id = CLVTP_PHONE_RHB_PR20,
   },
};
...
int __init sfi_handle_spid(struct sfi_table_header *table)
{
    ...
    if (spid.customer_id == 0xffff || spid.vendor_id == 0xffff){
        pr_info("warning:spid invalid - assign a default value \n");
        memset(&spid, 0, sizeof(struct soft_platform_id));
        spid = rhb_oemb_table.spid;
    }
