...
*/
        {"imx175", SOC_CAMERA, ATOMISP_CAMERA_PORT_PRIMARY},
...
int camera_sensor_csi_2(struct v4l2_subdev *sd, u32 port,
                          ...
                          u32 raw_format, u32 raw_bayer_order, int flag)
{
        ...
        if (flag) {
                ...
                csi->input_format = format;
                csi->raw_bayer_order = bayer_order;
                csi->raw_capture_input_format = raw_format;
                csi->raw_capture_bayer_order = raw_bayer_order;
                v4l2_set_subdev_hostdata(sd, (void *)csi);
                dev_info(&client->dev,
                         "camera pdata: port: %d lanes: %d order: %8.8x\n",
                         port, lanes, bayer_order);
					...
					struct devs_id *dev)
{
	...
	#if 1
	//if (!strcmp(i2c_info.type, "hm2056")){
	//»     i2c_info.addr = (0x78 >> 1); //fake value for adding node
	//»     strncpy(i2c_info.type, "gc2155", SFI_NAME_LEN);
	//»     printk("name:%s, addr=0x%x", i2c_info.type, i2c_info.addr);
	//}
	#endif
	//Clark--
	...
	i2c_info.platform_data = pdata;
        ...
        // i2c address
        if ((!strcmp(i2c_info.type, "imx175")) || (!strcmp(i2c_info.type, "gc2155"))){
                i2c_info.addr = 0x3C;
	...
	if (!strcmp(i2c_info.type, "gc0339")){
                i2c_info.addr = 0x21; //fake value for adding node
                printk("name:%s, addr=0x%x", i2c_info.type, i2c_info.addr);
        }
#endif
